...
/// Nth dimensional convex hull
///
/// This type is a type-safe wrapper around raw the qhull structures. As a result, many operations
/// involve walking linked lists and aren't particularly effecient. Prefer to use the other
/// `ConvexHull` type if possible.
#[derive(Debug)]
...
impl<N: DimName> ConvexHull<N> {
    ...
    unsafe fn init_shared(buf: *mut f64, len: usize) -> Self {
        ...
        *qhull_sys::rqh_get_NOerrexit(qh) = 0;
        let mut cmd = *b"qhull   Qc Qx         Qt\0";
...
impl<N: DimName> Drop for ConvexHull<N> {
    fn drop(&mut self) {
        unsafe {
            ...
            let mut tolong = 0;
            let mut currlong = 0;
            qhull_sys::qh_memfreeshort(
                self.qh.as_ptr(),
                ptr::addr_of_mut!(currlong),
                ptr::addr_of_mut!(tolong)
            );
...
impl<'a, N: DimName> Facet<'a, N> {
    ...
    pub fn offset(&self) -> f64 {
        unsafe {
            ptr::addr_of!((*self.ptr.as_ptr()).offset).read()
        }
    }

    pub fn normal(&self) -> &'a [f64] {
        unsafe {
            let normal_ptr = ptr::addr_of!((*self.ptr.as_ptr()).normal).read();
            std::slice::from_raw_parts(normal_ptr, N::dim() + 1)
        }
    }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(ptr) = self.ptr {
            unsafe {
                ...
                let next_ptr = NonNull::new(ptr::addr_of!((*ptr.as_ptr()).next).read());
                if next_ptr.is_none() {
                    return None
                }
                self.ptr = next_ptr;
...
/// Nth Delaunay triangulation.
///
/// This type is a type-safe wrapper around raw the qhull structures. As a result, many operations
/// involve walking linked lists and aren't particularly effecient. Prefer to use the other
/// `Delaunay` type if possible.
#[derive(Debug)]
          ...
          DimSum<N, U1>: DimName,
{
    ...
    unsafe fn init_shared(buf: *mut f64, len: usize) -> Self {
        ...
        qhull_sys::qh_init_A(qh, stdin, stdout, stderr, 0, ptr::null_mut());
        *qhull_sys::rqh_get_NOerrexit(qh) = 0;
        let mut cmd = *b"qhull d Qc Qx Qbb Q12 Qt Qz\0";
        qhull_sys::qh_initflags(qh, cmd.as_mut_ptr().cast());
    ...
    pub(crate) fn last_low(&self) -> f64 {
        unsafe {
            qhull_sys::rqh_get_last_low(self.cv.qh.as_ptr())
        }
    }

    pub(crate) fn last_high(&self) -> f64 {
        unsafe {
            qhull_sys::rqh_get_last_high(self.cv.qh.as_ptr())
        }
    }

    pub(crate) fn last_newhigh(&self) -> f64 {
        unsafe {
            qhull_sys::rqh_get_last_newhigh(self.cv.qh.as_ptr())
        }
    }

    pub fn find_best_simplex__<'a>(
        &'a self,
        point: OVector<f64, N>,
              ...
              DefaultAllocator: Allocator<f64, DimSum<N, U1>>,
    {
        ...
        'outer: for _cycle in 0..(solver_cache.0.len() / 4 + 1) {
            let mat = if let Some(solver) = &solver_cache.0[simplex.id() as usize] {
                &solver.mat
            } else {
            ...
            // TODO: Might this be more performant with a row-major matrix?
            let iter = mat.row_iter().zip(simplex.neighbors());
            for (row, neighbor) in iter {
                let coord = (row * &projected_point)[(0, 0)];
                if coord < -tolerance {
                    // Find the neighbor corresponding to the coord
                    if neighbor.is_upper_delaunay() {
                        // SciPy returns here, but it seems we often find a matching simplex if we
                        // attempt a bruteforce search instead. The performance trade-off being
                        // made is that we will take longer to detect if a point is outside the
                        // triangulation.
        ...
        let f = self.find_best_simplex_bruteforce__(point, solver_cache, tolerance);
        if let Some(f) = f {
            if let Some(hint) = hint {
                *hint = f;
            }
    ...
    fn find_best_simplex_bruteforce__<'a>(
        &'a self,
        point: OVector<f64, N>,
        solver_cache: &BarycentricSolverCache<N>,
        tolerance: f64,
              ...
              DefaultAllocator: Allocator<f64, DimSum<N, U1>>,
    {
        ...
        self.full_simplices()
            ...
            .filter_map(|simplex| {
                ...
                if let Some(solver) = &solver_cache.0[simplex.id() as usize] {
                    // let mut barycentric_coords = OVector::repeat(0.0);
                    // solver.solve_mut(point.clone(), &mut barycentric_coords);
                    let barycentric_coords = solver.solve(point.clone());
                    if barycentric_coords.iter().all(|f| *f >= -tolerance && *f <= 1.0 + tolerance) {
                        return Some(simplex)
                    }
                } else {
                    ...
                    for neighbor in simplex.neighbors() {
                        ...
                        let solver = if let Some(solver) = &solver_cache.0[neighbor.id() as usize] {
                            solver
                        } else {
                        ...
                        let barycentric_coords = solver.solve(point.clone());
                        let inside = neighbor.neighbors()
                            .zip(barycentric_coords.iter())
                            .all(|(inner_neighbor, coord)| {
                                if inner_neighbor == simplex {
                                    *coord >= -tolerance_sqrt && *coord <= 1.0 + tolerance
                                } else {
                                    *coord >= -tolerance && *coord <= 1.0 + tolerance
                                }
                            });
                        if inside {
              ...
              DefaultAllocator: Allocator<f64, DimSum<N, U1>>,
    {
        ...
        while changed && best_distance <= 0.0 {
            changed = false;
            // let mut iter = simplex.neighbors();
    ...
    pub fn compute_barycentric_solver_cache(&self) -> BarycentricSolverCache<N>
        where DefaultAllocator: Allocator<f64, DimSum<N, U1>, DimSum<N, U1>>,
              DefaultAllocator: Allocator<f64, DimSum<N, U1>>,
              DefaultAllocator: Allocator<(usize, usize), DimSum<N, U1>>,
              DefaultAllocator: Allocator<f64, nalgebra::DimMinimum<DimSum<N, U1>, DimSum<N, U1>>>,
              DimSum<N, U1>: nalgebra::DimMin<DimSum<N, U1>, Output = DimSum<N, U1>>,
    {
        ...
        for simplex in self.simplices() {
            ...
            let mat = OMatrix::from_iterator_generic(
                DimSum::<N, U1>::name(),
                DimSum::<N, U1>::name(),
                iter
            );
            ...
            let lu = mat.clone().lu();
            let mat = if let Some(mat) = lu.try_inverse() {
                mat
            } else {
                continue;
            };
...
pub struct BarycentricSolver<N: DimName>
    where N: DimAdd<U1>,
          DefaultAllocator: Allocator<f64, DimSum<N, U1>, DimSum<N, U1>>
{
    pub(crate) mat: OMatrix<f64, DimSum<N, U1>, DimSum<N, U1>>,
}
...
fn test_delaunay_4d_find_simplex() {
    ...
    // panic!("{:#?}", solver_cache.0[simplex.id() as usize]);
    let mut points = simplex.vertices().map(|v| v.point()).collect::<Vec<_>>();
    panic!("{:#?}", points);
    points.sort_by(|a, b| a.partial_cmp(b).unwrap());
    assert_eq!(points, [
